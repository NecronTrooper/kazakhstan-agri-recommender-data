...
Как найдено:
  ...
  Структура листа "Monthly Prices" подтверждена разовым диагностическим
  разбором при подготовке скрипта (не входит в репозиторий — при смене
  структуры файла источником сверяйтесь с validate_columns() ниже):
...
# Подтверждены разовым диагностическим разбором при подготовке скрипта
COLUMN_MAP = {
